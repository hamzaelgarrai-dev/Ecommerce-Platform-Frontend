...
const NavBar = () => {
	...
	return (
		<nav className='container mx-auto flex items-center my-2'>
			<div className='text-primary text-4xl font-bold'>Logo</div>
			<div className='flex-1 '>
				<div className='w-[80%] mx-auto flex items-center border-[1.5px] border-[#e1e1e6] rounded-full p-1'>
					<input
						type='text'
						value={inputValue}
			...
			<div className='flex items-center gap-2'>
				...
				{isAuthenticated ? (
					<div className='bg-white rounded-xl p-1.5 border-2 border-[#e1e1e6] z-0'>
						<User />
					</div>
				): (<button className="rounded-xl text-white bg-primary px-4 py-2 font-semibold">Log in</button>)}
				<div className='w-0.5 bg-[#e1e1e6] h-8 rounded-full'></div>
				<button
					onClick={() => setShowLangue((prev) => !prev)}
					onBlur={() => setShowLangue((prev) => !prev)}
				...
				>
					...
					<ChevronDown
						size={22}
						className={`${
							showLangue ? "rotate-180" : "rotate-0"
						} transition-transform duration-300 ease-in-out`}
					/>
					{showLangue && (
						<ul className='absolute top-9 right-0 shadow-[0_1px_6px_rgba(0,0,0,0.15)] rounded-xl px-4 py-2 z-50 text-black'>
